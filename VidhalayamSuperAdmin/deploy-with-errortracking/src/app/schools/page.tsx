...
export default function SchoolsPage() {
  ...
  const createSchool = async () => {
    ...
    try {
      ...
      // ── Step 1: Create Firebase Auth account for the principal ──
      const tempPassword = generateTempPassword(school_code);
      let adminUid = "";
      try {
        const cred = await createUserWithEmailAndPassword(secondaryAuth, form.adminEmail, tempPassword);
        adminUid = cred.user.uid;
        // Sign out the secondary app immediately — we don't need it anymore
        await signOut(secondaryAuth);
      } catch (authErr: any) {
        if (authErr.code === "auth/email-already-in-use") {
          alert(`❌ ${form.adminEmail} already has a Firebase account.\nUse a different email or reset their password manually.`);
          setSaving(false);
          return;
        }
      ...
      if (logoFile) {
        ...
        logoUrl = await uploadLogo(school_code);
        setUploadedLogoUrl(logoUrl);
      }

      // ── Step 3: Build WhatsApp config ──
      ...
      // ── Step 4: Save school to Firestore (includes adminUid) ──
      await addDoc(collection(db, "schools"), {
        school_id, school_code,
        name: form.school_name, schoolName: form.school_name,
        village: form.village, district: form.district, state: form.state, city: form.village,
        adminName:  form.adminName,
        adminEmail: form.adminEmail,
        adminPhone: form.adminPhone,
        adminUid,                           // ← links Firebase Auth user to this school
        logoUrl,
        primaryColor: form.primaryColor,
        tagline:      form.tagline,
        status:       "active",
        mustChangePassword: true,           // ← flag for app to prompt password change on first login
        createdAt: serverTimestamp(),
        ...(whatsappConfig ? { whatsappConfig } : {}),
      });

      // ── Step 5: Show credentials modal, then QR ──
      setShowModal(false);
      ...
      // QR will be shown after credentials are dismissed
  ...
  const resetAdminPassword = async (schoolId: string, adminEmail: string) => {
    ...
    setResetError("");
    try {
      // Call API to reset password in Firebase Auth
      const response = await fetch("/api/reset-admin-password", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: adminEmail, tempPassword }),
      });
